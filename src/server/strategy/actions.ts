 ...
 * `replaceStrategy` (one transaction) come first; briefing the coming week
 * is a separate model call afterwards, and a failure there is logged and
 * NOT fatal -- the user still lands on a complete strategy, and `/strategy`
 * offers the brief step again. The step is advanced only once the strategy
 * is durable, and `redirect()` stays outside every try, because it works by
 * throwing Next's navigation signal.
 */
...
async function briefWeek(
  ...
  try {
    const briefs = await draftWeek(strategy, business, voice, weekIndex)
...
export async function buildStrategy(): Promise<StrategyActionResult> {
  ...
  // Non-fatal by design: the strategy is durable, and /strategy offers this
  // step again if it did not complete here.
  const briefed = await briefWeek(userId, strategy, business, voice, profile.timezone)
  if (!briefed.ok) {
  ...
  // thing seen -- not the dashboard. revalidatePath so the (onboarded)
  // pages render the new plan rather than a cached empty state.
...
export async function briefUpcomingWeek(): Promise<StrategyActionResult> {
  ...
  const result = await briefWeek(userId, strategy, business, voice, profile.timezone)
  if (result.ok) {
    revalidatePath('/strategy')
    revalidatePath('/calendar')
    revalidatePath('/dashboard')
