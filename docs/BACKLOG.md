...
**Done, Milestone 4** (`refactor(env): split env into public and server…`). Five
call sites, changed in the same edit that replaced the Stripe keys with
Razorpay's — which is exactly the trigger this entry named.
...
**Done, Milestone 4** (`refactor(env): split env into public and server…`). The
trigger this entry named — "the first server secret that is genuinely sensitive
to name" — arrived with Razorpay. `src/lib/env.ts` is deleted; every import site
now takes one half or the other, and `src/app/(auth)/login/page.tsx` can no
longer see the name of a single secret.

`withoutBlanks` is duplicated across the two files rather than shared from a
third. Eight lines, against a module imported by both halves that would be a
standing path for a server import to creep back into the client-safe one.
